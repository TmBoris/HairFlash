...
class ModulationModuleWithShapeS(nn.Module):
    def __init__(self, layernum, last=False, inp=512, middle=512):
        super().__init__()
        self.layernum = layernum
        self.last = last
    ...
    def forward(self, x, shape_add, embedding):
        x = self.fc(torch.cat((x, shape_add), dim=-1))
        x = self.norm(x)
        gamma = self.gamma_function(embedding)
        beta = self.beta_function(embedding)
        out = x * (1 + gamma) + beta
...
class ClipBlendingModelWithShapeS(nn.Module):
    def __init__(self):
        super().__init__()
        self.pixelnorm = PixelNorm()
        self.clip_model = torch.load('/home/bspanfilov/HairFlash/pretrained_models/ViT-B-32_model.pt')[0].to('cuda')
        self.transform = T.Compose(
            [T.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))])
        self.face_pool = torch.nn.AdaptiveAvgPool2d((224, 224))
        self.modulation_module_list = nn.ModuleList(
            [ModulationModuleWithShapeS(12, i == 4, inp=512 * 3, middle=1024) for i in range(5)]
        )
    ...
    def forward(self, latent_face, latent_color, target_face, hair_color, latent_shape):
        ...
        # по сути прямо вот сюда нужно пихать получение эмбеддингов из волос 
        # shape картинки и просто передавать их в латент_in 
        # надо не забыть масочку наложить, чтобы волосы там выделить, ну и будем надеяться, что оно поможет
        latent_in = torch.cat((latent_color, embed_face, embed_color), dim=-1)

        dt_latent = self.pixelnorm(latent_face)
        dt2_latent = self.pixelnorm(latent_shape)
        for modulation_module in self.modulation_module_list:
            dt_latent = modulation_module(dt_latent, dt2_latent, latent_in)
        output = latent_face + 0.1 * dt_latent
        return output
    

class ClipBlendingModelWithShapeSv1(nn.Module):
    def __init__(self):
        super().__init__()
        self.pixelnorm = PixelNorm()
        self.clip_model = torch.load('/home/bspanfilov/HairFlash/pretrained_models/ViT-B-32_model.pt')[0].to('cuda')
        ...
        self.face_pool = torch.nn.AdaptiveAvgPool2d((224, 224))
        self.modulation_module_list = nn.ModuleList(
            [ModulationModule(12, i == 4, inp=512 * 4, middle=1024) for i in range(5)]
        )
    ...
    def forward(self, latent_face, latent_color, target_face, hair_color, latent_shape):
        ...
        # shape картинки и просто передавать их в латент_in 
        # надо не забыть масочку наложить, чтобы волосы там выделить, ну и будем надеяться, что оно поможет
        latent_in = torch.cat((latent_color, embed_face, embed_color, latent_shape), dim=-1)
...
class ClipBlendingModelWithShapeImgEmbdsv1(nn.Module):
    def __init__(self):
        super().__init__()
        self.pixelnorm = PixelNorm()
        self.clip_model = torch.load('/home/bspanfilov/HairFlash/pretrained_models/ViT-B-32_model.pt')[0].to('cuda')
    ...
    def forward(self, latent_face, latent_color, target_face, hair_color, hair_shape):
        embed_face = self.get_image_embed(target_face).unsqueeze(1).expand(-1, 12, -1)
        embed_color = self.get_image_embed(hair_color).unsqueeze(1).expand(-1, 12, -1)
        embed_shape = self.get_image_embed(hair_shape).unsqueeze(1).expand(-1, 12, -1)


        # по сути прямо вот сюда нужно пихать получение эмбеддингов из волос 
        # shape картинки и просто передавать их в латент_in 
        # надо не забыть масочку наложить, чтобы волосы там выделить, ну и будем надеяться, что оно поможет
        latent_in = torch.cat((latent_color, embed_face, embed_color, embed_shape), dim=-1)
...
class ClipBlendingModelWithShapeImgEmbds(nn.Module):
    def __init__(self):
        super().__init__()
        self.pixelnorm = PixelNorm()
        self.clip_model = torch.load('/home/bspanfilov/HairFlash/pretrained_models/ViT-B-32_model.pt')[0].to('cuda')
    ...
    def forward(self, latent_face, latent_color, target_face, hair_color, hair_shape):
        ...
        dt_latent = self.pixelnorm(latent_face)
        dt2_latent = self.pixelnorm(embed_shape)
        for modulation_module in self.modulation_module_list:
            dt_latent = modulation_module(dt_latent, dt2_latent, latent_in)
        output = latent_face + 0.1 * dt_latent
        return output
