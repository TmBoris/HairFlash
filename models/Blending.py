...
from models.Encoders import ClipBlendingModel, PostProcessModel, ClipBlendingModelWithShapeS, ClipBlendingModelWithShapeImgEmbds, ClipBlendingModelWithShapeSv1, ClipBlendingModelWithShapeImgEmbdsv1
...
class Blending(nn.Module):
    ...
    def __init__(self, opts, net=None):
        ...
        self.blending_encoder = ClipBlendingModel()
        self.blending_encoder.load_state_dict(torch.load(self.opts.blending_checkpoint)['model_state_dict'])
...
class Blending1WithoutColorMask(nn.Module):
    """
    Module for transferring the desired hair color and post processing
    """
    ...
    @torch.inference_mode()
    def blend_images(self, align_shape, name_to_embed, **kwargs):
        I_1 = name_to_embed['face']['image_norm_256']
        I_2 = name_to_embed['shape']['image_norm_256']
        I_3 = name_to_embed['color']['image_norm_256']
...
class Blending2WithShapeS(nn.Module):
    """
    Module for transferring the desired hair color and post processing
    """
    ...
    def __init__(self, opts, net=None):
        ...
        self.blending_encoder = ClipBlendingModelWithShapeS()
        self.blending_encoder.load_state_dict(torch.load(self.opts.blending_checkpoint)['model_state_dict'])
        self.blending_encoder.to(self.opts.device).eval()
    ...
    def blend_images(self, align_shape, align_color, name_to_embed, **kwargs):
        ...
        latent_S_2 = name_to_embed['shape']["S"]
        latent_S_3 = name_to_embed['color']["S"]
        ...
        # Blending
        if I_1 is not I_3 or I_1 is not I_2:
            S_blend_6_18 = self.blending_encoder(latent_S_1[:, 6:], latent_S_3[:, 6:], I_1 * target_mask, I_3 * HM_3E, latent_S_2[:, 6:])
            S_blend = torch.cat((latent_S_1[:, :6], S_blend_6_18), dim=1)
        else:
            S_blend = latent_S_1
...
class Blending2WithShapeImg(nn.Module):
    """
    Module for transferring the desired hair color and post processing
    """
    ...
    def __init__(self, opts, net=None):
        ...
        self.blending_encoder = ClipBlendingModelWithShapeImgEmbds()
        self.blending_encoder.load_state_dict(torch.load(self.opts.blending_checkpoint)['model_state_dict'])
        self.blending_encoder.to(self.opts.device).eval()
    ...
    def blend_images(self, align_shape, align_color, name_to_embed, **kwargs):
        ...
        mask_de = self.dilate_erosion.hair_from_mask(
            torch.cat([name_to_embed[x]['mask'] for x in ['face', 'color', 'shape']], dim=0)
        )
        HM_1D, _ = mask_de[0][0].unsqueeze(0), mask_de[1][0].unsqueeze(0)
        HM_3D, HM_3E = mask_de[0][1].unsqueeze(0), mask_de[1][1].unsqueeze(0)
        _, HM_2E = mask_de[0][2].unsqueeze(0), mask_de[1][2].unsqueeze(0)
        ...
        # Blending
        if I_1 is not I_3 or I_1 is not I_2:
            S_blend_6_18 = self.blending_encoder(latent_S_1[:, 6:], latent_S_3[:, 6:], I_1 * target_mask, I_3 * HM_3E, I_2 * HM_2E)
            S_blend = torch.cat((latent_S_1[:, :6], S_blend_6_18), dim=1)
        else:
            S_blend = latent_S_1
...
class Blending3WithBWShapeImg(nn.Module):
    """
    Module for transferring the desired hair color and post processing
    """
    ...
    def blend_images(self, align_shape, align_color, name_to_embed, **kwargs):
        ...
        # Blending
        if I_1 is not I_3 or I_1 is not I_2:
            I_2_gray = self.gray_transform(I_2)
            S_blend_6_18 = self.blending_encoder(latent_S_1[:, 6:], latent_S_3[:, 6:], I_1 * target_mask, I_3 * HM_3E, torch.cat((I_2_gray, I_2_gray, I_2_gray), dim=1) * HM_2E)
            S_blend = torch.cat((latent_S_1[:, :6], S_blend_6_18), dim=1)
        else:
            S_blend = latent_S_1
...
class Blendingv1S(nn.Module):
    ...
    def __init__(self, opts, net=None):
        ...
        self.blending_encoder = ClipBlendingModelWithShapeSv1()
        self.blending_encoder.load_state_dict(torch.load(self.opts.blending_checkpoint)['model_state_dict'])
        self.blending_encoder.to(self.opts.device).eval()
...
class Blendingv1ImgEmbeds(nn.Module):
    """
    Module for transferring the desired hair color and post processing
    """
    ...
    def __init__(self, opts, net=None):
        ...
        self.blending_encoder = ClipBlendingModelWithShapeImgEmbdsv1()
        self.blending_encoder.load_state_dict(torch.load(self.opts.blending_checkpoint)['model_state_dict'])
        self.blending_encoder.to(self.opts.device).eval()
