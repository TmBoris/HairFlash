...
class Trainer:
    def __init__(self,
        ...
        self.seg.load_state_dict(torch.load('pretrained_models/seg.pth'))
    ...
    def save_model(self, name, save_online=True):
        with TemporaryDirectory() as tmp_dir:
            torch.save({'model_state_dict': self.model.state_dict()}, f'{tmp_dir}/{name}.pth')
    ...
    def calc_loss(self, I_gen, I_face, I_color, mask_face, mask_hair, gen_hair):
        ...
        gen_embed = self.model.get_image_embed(I_gen * gen_hair)
