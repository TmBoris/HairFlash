...
class Trainer:
    ...
    def calc_loss(self, I_gen, I_face, I_shape, I_color, mask_face, mask_hair_color, mask_hair_shape, gen_hair):
        ...
        horizontal_shape_loss = F.mse_loss(horizontal_sobel_gen, horizontal_sobel_shape)
        vertical_shape_loss = F.mse_loss(vertical_sobel_gen, vertical_sobel_shape)

        shape_loss = torch.sqrt((horizontal_shape_loss + vertical_shape_loss) / 2) / 2
