...
class Trainer:
    ...
    def calc_loss(self, I_gen, I_face, I_shape, I_color, mask_face, mask_hair_color, mask_hair_shape, gen_hair):
        ...
        gen_embed = self.model.module.get_image_embed(torch.cat((I_gen_gray, I_gen_gray, I_gen_gray), dim=1) * gen_hair)
        gt_embed = self.model.module.get_image_embed(torch.cat((I_shape_gray, I_shape_gray, I_shape_gray), dim=1) * mask_hair_shape)
        shape_loss = F.mse_loss(gen_embed, gt_embed)
