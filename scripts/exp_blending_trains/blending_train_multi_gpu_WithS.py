...
from models.Encoders import ClipBlendingModelWithShapeS as BlendingModel
...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for batch in tqdm(self.train_dataloader):
            color_s, shape_s, align_s, align_f, color_i, face_i, target_mask, HM_3E, HM_XE = map(lambda x: x.to(self.device),
                                                                                        batch)
            bsz = color_s.size(0)

            blend_s = self.model(align_s[:, 6:], color_s[:, 6:], face_i * target_mask, color_i * HM_3E, shape_s[:, 6:])
...
def prepare_item(exp, path, files_suffix=SUFFIX):
    ...
    try:
        ...
        shape_path = os.path.join(path, 'FS', im2 + files_suffix)
        Shape_S = torch.from_numpy(np.load(shape_path)['latent_in']).squeeze(0)
        ...
        return (Color_S, Align_S, Align_F, Color_I, Face_I, Shape_S)
    except:
        return None
...
class Blending_dataset(Dataset):
    def __init__(self, exps, path):
        ...
        self.items = []
        with torch.no_grad():
            for (Color_S, Align_S, Align_F, Color_I, Face_I, Shape_S) in tqdm(tmp_dataloader):
