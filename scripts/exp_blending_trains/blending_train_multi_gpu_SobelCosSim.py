...
class Trainer:
    ...
    def calc_loss(self, I_gen, I_face, I_shape, I_color, mask_face, mask_hair_color, mask_hair_shape, gen_hair):
        ...
        gt_embed = self.model.module.get_image_embed(I_color * mask_hair_color)
        hair_loss = (1 - F.cosine_similarity(gen_embed, gt_embed)).mean()

        print('I_gen.shape = ', I_gen.shape)
        print('I_shape.shape = ', I_shape.shape)

        print('I_gen type, dtype: ', type(I_gen), I_gen.dtype)
        print('I_shape type, dtype: ', type(I_shape), I_shape.dtype)

        # to grayscale
        I_gen_gray = self.gray_transform(I_gen)
        I_shape_gray = self.gray_transform(I_shape)

        # creating filters
        sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(I_gen_gray.device)
        sobel_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(I_gen_gray.device)

        # applying sobel_x, concat back to 3 channels
        horizontal_sobel_gen = F.normalize(F.conv2d(I_gen_gray, sobel_x, padding=1))
        horizontal_sobel_gen = torch.cat((horizontal_sobel_gen, horizontal_sobel_gen, horizontal_sobel_gen), dim=1)
        horizontal_sobel_shape = F.normalize(F.conv2d(I_shape_gray, sobel_x, padding=1))
        horizontal_sobel_shape = torch.cat((horizontal_sobel_shape, horizontal_sobel_shape, horizontal_sobel_shape), dim=1)

        # applying sobel_y, concat back to 3 channels
        vertical_sobel_gen = F.normalize(F.conv2d(I_gen_gray, sobel_y, padding=1))
        vertical_sobel_gen = torch.cat((vertical_sobel_gen, vertical_sobel_gen, vertical_sobel_gen), dim=1)
        vertical_sobel_shape = F.normalize(F.conv2d(I_shape_gray, sobel_y, padding=1))
        vertical_sobel_shape = torch.cat((vertical_sobel_shape, vertical_sobel_shape, vertical_sobel_shape), dim=1)

        gen_embed = self.model.module.get_image_embed(horizontal_sobel_gen * gen_hair)
        gt_embed = self.model.module.get_image_embed(horizontal_sobel_shape * mask_hair_shape)
        horizontal_shape_loss = (1 - F.cosine_similarity(gen_embed, gt_embed)).mean()

        gen_embed = self.model.module.get_image_embed(vertical_sobel_gen * gen_hair)
        gt_embed = self.model.module.get_image_embed(vertical_sobel_shape * mask_hair_shape)
        vertical_shape_loss = (1 - F.cosine_similarity(gen_embed, gt_embed)).mean()

        shape_loss = (horizontal_shape_loss + vertical_shape_loss) / 2

        losses = {'face loss': face_loss, 'hair loss': hair_loss, 'shape loss': shape_loss, 'loss': face_loss + hair_loss + shape_loss}
    ...
    def train_one_epoch(self):
        ...
        for batch in tqdm(self.train_dataloader):
            ...
            # I_shape_gray = self.gray_transform(shape_i)
            blend_s = self.model(align_s[:, 6:], color_s[:, 6:], face_i * target_mask, color_i * HM_3E,
                                  shape_i * HM_2E)
                                #   torch.cat((I_shape_gray, I_shape_gray, I_shape_gray), dim=1) * HM_2E)
            ...
            loss, info = self.calc_loss(self.downsample_256(I_G), face_i, shape_i, color_i, target_mask, HM_3E, HM_2E, HM_XE)
    ...
    def validate(self):
        ...
        for batch in tqdm(self.test_dataloader):
            ...
            _, info = self.calc_loss(self.downsample_256(I_G), face_i, shape_i, color_i, target_mask, HM_3E, HM_2E, HM_XE)
